...
<?php 
      ...
      $sql_d = "select `nama`, `email` from `admin` 
    ...
    <div class="card card-info">
      ...
      <!-- form start -->
      <div class="col-sm-10">
	   <?php if((!empty($_GET['notif']))&&(!empty($_GET['data']))){?>
	      <?php if($_GET['notif']=="editkosong"){?>
	          <div class="alert alert-danger" role="alert">Maaf data 
	          <?php echo $_GET['data'];?> wajib di isi</div>
              <?php }?>
          <?php }?>
        </div>
        <form class="form-horizontal" method="post" 
      action="index.php?include=konfirmasi_edit_profil">
        <div class="card-body">
            <div class="form-group row">
                <label for="foto" class="col-sm-12 col-form-label">
                <span class="text-info">
                <i class="fas fa-user-circle"></i> <u>
                PROFIL MAHASISWA</u></span></label>
            </div>
          <div class="form-group row">
